...
The format of gene annotation file. Default GTF (Gene Transfer Format). For job B<count> only. 
In short, from UCSC Table Browser, the 'knownGene' table with all fields is in GPF (Gene Prediction Format) format and 
